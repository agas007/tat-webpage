...
import { Plus, MoreHorizontal, Search, Filter, AlignLeft, Calendar } from "lucide-react";
...
export default function TasksBoard() {
  ...
  const tasks = [
    { id: 1, title: "SPT Masa PPN - PT Krakatau Steel", client: "PT Krakatau Steel", status: "todo", assignee: "Livia", due: "15 Oct", priority: "High" },
    { id: 2, title: "Review TP Doc (Local File)", client: "PT Saratoga", status: "review", assignee: "Ari S.", due: "18 Oct", priority: "High" },
    { id: 3, title: "Laporan Keuangan Q3", client: "PT Indopelita", status: "done", assignee: "Adib M.", due: "05 Oct", priority: "Medium" },
    { id: 4, title: "Pendampingan Pemeriksaan (Minggu 3)", client: "PT Angkasa Pura", status: "todo", assignee: "Vitha", due: "20 Oct", priority: "High" },
    { id: 5, title: "SPT Tahunan OP - Bapak Direktur Utama", client: "Rukun Raharja", status: "progress", assignee: "Jihan", due: "25 Oct", priority: "Low" },
    { id: 6, title: "Penyusunan Keberatan Pajak", client: "PLN Enjiniring", status: "progress", assignee: "Mutsaqoful", due: "22 Oct", priority: "High" },
  ];
              ...
              <div className="flex-1 min-h-[200px] flex flex-col gap-3">
                {tasks.filter(t => t.status === col.id).map(task => (
                  <div key={task.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all cursor-grab group">
                    ...
                    <p className="text-xs text-brand-blue font-medium mb-3 truncate">{task.client}</p>
                    
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-brand-silver flex items-center justify-center text-[10px] font-bold text-brand-dark border border-white shadow-sm" title={task.assignee}>
            ...
            <tbody className="divide-y divide-gray-100">
              {tasks.map((task) => (
                <tr key={task.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-brand-blue text-sm">{task.client}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500 flex items-center gap-1.5">
                    <Calendar size={14} className="text-gray-400" /> {task.due}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
                        task.priority === 'High' ? 'bg-red-50 text-red-600' :
